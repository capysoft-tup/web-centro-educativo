...
const ROLES_INFO = {
  Staff: {
    ...
    permissions: ['cargar_asistencias', 'registrar_notas', 'publicar_anuncios_aula', 'ver_agenda_clases']
  },
  'Padre/Tutor': {
    label: 'Padre/Tutor',
    icon: 'family_restroom',
    displayLabel: 'PADRE/TUTOR',
    permissions: ['ver_legajo_hijo', 'autorizar_eventos', 'ver_estado_cuotas', 'comunicarse_con_docente']
  },
  Estudiante: {
    label: 'Estudiante',
    icon: 'school',
    displayLabel: 'ESTUDIANTE',
    permissions: ['ver_mis_notas', 'ver_mis_asistencias', 'descargar_material', 'ver_horarios']
  }
};
...
const Login = () => {
  ...
  if (isLoggedIn && user) {
    // Determine the role for the logged-in user (fallback to Estudiante if role is not recognized)
    const activeRoleKey = user.role in ROLES_INFO ? user.role : 'Estudiante';
        ...
        <main className="flex-1 flex items-center justify-center p-4 sm:p-8 md:p-16">
          <div className="w-full max-w-2xl bg-white rounded-xl shadow-[0px_20px_40px_rgba(35,44,81,0.06)] border border-slate-100 p-8 sm:p-12 text-center transition-all duration-300 hover:shadow-[0px_25px_50px_rgba(35,44,81,0.1)]">
            ...
            <h1 className="font-headline font-bold text-2xl sm:text-3xl text-slate-800 tracking-tight mb-4">
              HOLA {currentRole?.displayLabel || user.role.toUpperCase()}, TIENES ESTOS PERMISOS:
            </h1>
            ...
            <div className="bg-orange-50/50 border border-orange-100 rounded-2xl p-6 sm:p-8 my-6 text-left max-w-md mx-auto">
              <ul className="space-y-4">
                {currentRole?.permissions.map((permission, index) => (
                  <li key={index} className="flex items-center gap-3 text-slate-700 text-base font-medium">
                    <span className="material-symbols-outlined text-orange-600 font-bold bg-orange-100 rounded-full p-1 text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>
                      check
                    </span>
                    {permission}
                  </li>
          ...
          <div className="relative z-10">
            <div className="mb-8">
              <img
                ...
                src={logo}
                onError={(e) => {
                  e.target.src = "https://lh3.googleusercontent.com/aida/ADBb0ugJ1I9qKGq-LA1zkyP0mAJ6KMVoto-k2Bhko7jS4BgxauPND3UeAT0onvR1k3ArEIe80EYEIMFR8Bwmy9Wq6vif1Hl7DAh1-BrEutL_eg8cYRnUjvE5tspD46XGp7iF9rIOZa8T-GNoMMlJIBDJIzgLlyIj-mRW9JHn4QnEsHs-VN4XmpP7DLXTLi0_GmkFUlL2utzr7RRQS131G6KzO_u524R8pUernkXd1WN1UmvTuth2AT7diNd-5Yg";
                }}
              />
            </div>
            <h1 className="font-headline font-extrabold text-5xl leading-tight tracking-tight text-white mb-6">
              Bienvenido a<br />Educar para<br />Transformar.
            </h1>
            <p className="font-body text-lg text-white/90 max-w-md">
              Transformando la interfaz educativa tradicional en una experiencia digital de nivel editorial para estudiantes, padres y profesores.
            </p>
          </div>
          
          <div className="relative z-10 mt-auto bg-white/20 backdrop-blur-md rounded-xl p-6 border border-white/30 shadow-sm">
            <div className="flex items-center gap-4">
              <span className="material-symbols-outlined text-lime-300" style={{ fontVariationSettings: "'FILL' 1" }}>
                lightbulb
              </span>
        ...
        <div className="w-full lg:w-1/2 p-8 sm:p-12 xl:p-16 flex flex-col justify-center bg-white relative">
          ...
          <div className="max-w-md w-full mx-auto">
            ...
            <div className="lg:hidden mb-10 text-center flex flex-col items-center mt-6">
              ...
              />
              <h2 className="font-headline font-bold text-3xl text-slate-800 tracking-tight">Portal de Acceso</h2>
            </div>

            <div className="hidden lg:block mb-10">
              <h2 className="font-headline font-bold text-4xl text-slate-800 tracking-tight mb-2">Portal de Acceso</h2>
              <p className="font-body text-slate-500">Inicia sesión para continuar tu viaje.</p>
            </div>
              ...
              <div className="relative">
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-orange-600 bg-orange-100 rounded-lg p-1.5 text-xl flex items-center justify-center" style={{ fontVariationSettings: "'FILL' 1" }}>
                          ...
                          <div className="flex items-center gap-3">
                            <span 
                              className={`material-symbols-outlined rounded-lg p-1 text-lg flex items-center justify-center transition-colors ${
                                isSelected ? 'text-orange-600 bg-orange-100' : 'text-slate-400 bg-slate-100'
                              }`} 
                              style={{ fontVariationSettings: isSelected ? "'FILL' 1" : "'FILL' 0" }}
                            >
                              {role.icon}
                            </span>
                            <span className="font-label text-sm font-semibold">{role.label}</span>
                          </div>
                          {isSelected && (
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-6">
              {/* ID / Email Field */}
              <div>
                <label className="block font-label text-sm font-medium text-slate-600 mb-2" htmlFor="identifier">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block font-label text-sm font-medium text-slate-600" htmlFor="password">
                    Contraseña
                  </label>
                  <a className="font-label text-sm font-medium text-orange-600 hover:text-orange-700 transition-colors" href="#">
              ...
              <div className="flex items-center">
                ...
                />
                <label className="ml-3 block font-body text-sm text-slate-600 cursor-pointer select-none" htmlFor="remember-me">
            ...
            <div className="mt-8 text-center">
              <p className="font-body text-sm text-slate-500">
                ¿Necesitas ayuda para acceder a tu cuenta? <br className="sm:hidden" />
                <a className="font-medium text-orange-600 hover:text-orange-700 transition-colors" href="#">
                  Contactar Soporte
                </a>
              </p>
            </div>
